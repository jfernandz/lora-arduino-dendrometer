...
So, as can be seen, in order to prepare the payload firstly the analog pin is stored in \cmd{potVal} variable, after this the function \cmd{lpp.reset()} is ued to reset the CayenneLPP buffer, then the measure \cmd{potVal} is added to the CayenneLPP buffer in the first channel using the function \cmd{lpp.addAnalogInput()}, finally the  \doubt{REDO}
