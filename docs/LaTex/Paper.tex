...
Another important consideration inside TTN platform is to check which decoder is chosen for the received payloads, this can be checked by clicking on \cmd{Payload Formats} tab inside Application Overview. As can be seen in figure \ref{fig:CayenneLPP_decoder}, it is so importan to choose the \cmd{CayenneLPP} decoder, because as indicated, the payload is being encoded in the end-nodes using the CayenneLPP standard. 
...
\begin{itemize}
  \item In a first place the reader will need to create a new device for the \textbf{IoTagent-LoRaWAN}, this
\end{itemize}
